...
function pact(command, argv) {
  ...
  const karmaServer = require('./utils/karma-utils');
  ...
  portfinder.getPorts(skyPagesConfig.skyux.pacts.length + 1, {}, (err, ports) => {
    ...
    skyPagesConfig.skyux.pacts.forEach((config, index) => {
      const options = JSON.parse(JSON.stringify(config));
      options.port = options.port || ports[index];
      options.host = options.host || 'localhost';
      pactServers.savePactServer(options);
    });
    ...
    return karmaServer.run(command, argv, 'src/app/**/*.pact-spec.ts')
      .then(exitCode => {
