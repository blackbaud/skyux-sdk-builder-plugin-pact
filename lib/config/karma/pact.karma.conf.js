...
function getConfig(config) {
  ...
  const sharedKarmaConfigPath = `@skyux-sdk/builder/config/karma/${ argv.watch ? 'watch' : 'test' }.karma.conf`;
  require(sharedKarmaConfigPath)(config);
  ...
  for (const pattern in config.preprocessors) {
    if (pattern.indexOf('spec-bundle.js') > -1) {
      preprocessors[pactPattern] = config.preprocessors[pattern];
      ...
      preprocessors[pattern] = config.preprocessors[pattern];
    }
  }
